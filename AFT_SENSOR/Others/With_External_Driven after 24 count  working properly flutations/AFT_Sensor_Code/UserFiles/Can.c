...
{	 
		...
		if(SET_BAUD_RATE ==  e250)
		{
			C0GMCS	= 0x01;	   
			C0BRP = 0x07;		
			C0BTR	= 0x0104;			 
		}	
		else if(SET_BAUD_RATE ==  e500)
		{
			C0GMCS	= 0x01;	   
			C0BRP = 0x03;  
			C0BTR	= 0x0104;	
		}
		else if(SET_BAUD_RATE ==  e1000)
		{
			C0GMCS	= 0x01; 		   
			C0BRP = 0x01;  
			C0BTR	= 0x0104;
...
void vTxMsgBufProcessing(u32 ID_msg_rx ,u8 tx_msg_DLC,u8* tx_msg_data,bool TxType)
{
	...
	u16 BuzzerCheckingTimeOut = 0;
		...
		if(TxType == UDS_TX)
		{
			...
			BuzzerCheckingTimeOut = u32GetMilTick();
			...
			while(BuzzerCheckingFlag == TRUE)
			{
				...
				if(u32GetMilTick() - BuzzerCheckingTimeOut > 100)
				{
				  BuzzerCheckingFlag = FALSE;
				}
			}
