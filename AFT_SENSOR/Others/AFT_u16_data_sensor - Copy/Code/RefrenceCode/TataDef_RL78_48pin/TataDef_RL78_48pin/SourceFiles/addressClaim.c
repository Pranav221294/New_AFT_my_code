...
#define  MY_INDUSTRY_GROUP                  (u8)0        	// J1939 TABLE B1: 1 = On-Highway Equipment
#define  MY_SYSTEM_INSTANCE                 (u8)0          	// First instance
#define  MY_SYSTEM                          (u8)0         	// J1939 TABLE B12: Instrumentation General System = 80
...
#define  MY_MANUFACTURER_CODE               (u16)8     		// Veethree (11 bit max)
#define  MY_IDENTITY_NUMBER                 (u32)0x00000 	// 21 bits
...
typedef struct
{
    ...
    struct
    {
        u8  IdentityNumber_u83     :5;  // 4:0 most significant 5 bits of identity number
        u8  ManufacturerCode_u83   :3;  // 7:5 least significant 2 bits of manufacturing code
    } u83;
    u8  ManufacturerCode_u84;           // msb of manufacturing code
    struct
    {
        u8  EcuInstance              :3;  // 2:0 ECU instance
        u8  FunctionInstance         :5;  // 7:3 function instance
    } u85;
    u8  DeviceFunction;                   // function
    struct
    {
        u8  Reserved                 :1;  // 0 reserved
        u8  VehicleSystem            :7;  // 7:1 industry group
    } u87;
    struct
    {
        u8  VehicleSystemInstance    :4;  // 3:0 vehicle system instance
        u8  IndustryGroup            :3;  // 6:4 industry group
        u8  ArbitraryAddressCapable  :1;  // 7 arbitrary address capable
    } u88;
...
static void _vAddressClaimInit(void)
{
    ...
    sMyName.u83.ManufacturerCode_u83  = (u8)(MY_MANUFACTURER_CODE & 0xFF);
...
void vRxISOaddressClaim(sJ1939_RX_MSG_TYPE *msg)
{
    ...
    u8 *myNamePtr, *RxNamePtr, nameCount;
...
static bool _bTxISOaddressClaim(u8 DA, u8 SA)
{
    ...
    // Transmit the messasge
	 vTxMsgBufProcessing(0x18EEFF17 ,8,(u8 *)&sMyName,J1939_TX); 
    return;
