...
const u8 dtcSupportedId[eNumofSupportedDTC+3][3]={													
													...
													{0xa1,0x84,0x11},//OilPressureShortGnd
...
#define CHECK_PROG_DEP 						0xFF01
#define CRC_VERIFICATION					0xFF03
#define SERVICE_RESET						0x3F00


#define REQUEST_TRANSFER_EXIT				0x37

#define ADD_LENGHT_FORMATE_IDENTIFIER		0x33
#define DATA_FORMATE_IDENTIFIER				0x00
...
#define LID_CAN_SPEEDO_FACTOR				0x55
...
#define DTE_MUL_FACTOR_MIN 					5
#define DTE_MUL_FACTOR_MAX 					15
...
#define IO_CONTROL_GAUGES_BYTE		 		(9 + 1) // 1 extra byte for control option 

#define MAX_NUM_BLOCK_LENGHT				0x0102
#define APPLICATION_START_ADD				0xF400
#define APPLICATION_END_ADD					0x3FFFF
#define NO_OF_BYTE_OF_APPLICATION			0x30C00 //(APPLICATION_END_ADD - APPLICATION_END_ADD) + 1
...
//Eargo 24V***********************************************************************************************************
	...
	const u8 u8TmlEcuSoftwareNumber[TML_ECU_SOFTWARE_NUMBER_BYTES] = {'2','7','5','4','1','6','3','0','1','1','1','0','_','N','R'};
...
//Eargo 12V***********************************************************************************************************	
	...
	const u8 u8TmlEcuSoftwareNumber[TML_ECU_SOFTWARE_NUMBER_BYTES] = {'2','7','5','4','1','6','3','0','1','1','1','1','_','N','R'};
...
const u16  u16ECUSoftwareVersionNum = 0x0100; //V01.00  
// released software SVN no. (used for internal purpose only)
const u8 u8IILSvnNo[IIL_SVN_NUMBER_BYTES] = {'1','2','6','1'}; 
...
//global variables 
static bool bRequestDownloadServiceActive = 0;
...
//Must keep u8BlockSequenceCounter = 1 becouse ,trasfer data service start block from 1
static u8 u8BlockSequenceCounter = 1;				
static bool bTranferDataStatusPending = 0;
static u16 BlockCounter = 0;
static u8 u8BlockNumber = 0;

static u8 dataTxBuffer[MAX_LENGTH];//it should actually not contain data more than MAX_DATA_LENGTH
...
static const u8 IILAESCipherKey[17] = "IIL1234567890123";  //BSVI EARGO
...
void startDiagnosticsSession(void) 
{
	...
	switch(singleFrameRx.dataBytes[2])
	{
		...
		case eProgrammingSession: // this case should be on top because its negative response code has higher priority
			//Programming session will support only in bootloader code 				
...
void ECUResetService(void)
{
	...
	switch(singleFrameRx.dataBytes[2])
	{
	 ...
	 if((conditionNotCorrect == TRUE || oldRPM != 0 || speed != 0) 
	 	&& receivedMessageError == FALSE )
	{
		diagnosticsResponse.u8data[3] = ConditionsNotCorrect;
		receivedMessageError = TRUE;
	}
	...
	else if(securityAccessInfo.serverLockStatus == eServerLocked)
	{
			...
			&& securityAccessInfo.securitySequence != eNone)//this condition may not be required
		{
			...
			else
			{
			   //extract the decrypted data
			    getEncryptedData();
				data_decrypt(encryptedReceivedData,IILAESCipherKey);
				if((memcmp(encryptedReceivedData,&seed,16)== ERR_OK) )
				{
					securityAccessInfo.serverLockStatus = eServerUnlocked;
					securityAccessInfo.IilserverLockStatus = eIILServerUnlocked;
					diagnosticsResponse.u8data[0] = 2;
					diagnosticsResponse.u8data[1] = 0x67;
					diagnosticsResponse.u8data[2] = eSendKey;
					diagnosticsResponse.bDataRdy = TRUE;
...
void communicationControlService(void)
{
	...
	switch(singleFrameRx.dataBytes[2]) //control type
	{
		...
		case eDisableRxandTx: 
			...
			break;
		default:
			if(diagnosticsResponse.u8data[3] != ServiceNotSupportedInActiveSession)
			{
				 diagnosticsResponse.u8data[3] = SubfunctionNotSupported;
				 receivedMessageError = TRUE;
			}
...
void ControlDTCSettingServiceMultiple(void)
{
	...
	if( numOfDTCs > eNumofSupportedDTC )
	{
		diagnosticsResponse.u8data[3] = RequestOutOfRange;
		receivedMessageError = TRUE;
	}
...
void ClearDiagnosticsInformationService(void)
{
	...
	else
	{
		...
		//No match was found, responce RequestOutOfRange ?	
		if(i == eNumofSupportedDTC )
		{	
			if( singleFrameRx.dataBytes[2] == 0xFF 
				&&  singleFrameRx.dataBytes[3] == 0xFF
				&& singleFrameRx.dataBytes[4] == 0xFF
			)
			{
				for(i=0;i<eNumofSupportedDTC;i++)
				{
...
void RequestDownload()
{
	...
	if(temp == eApplicationMode)
	{
		diagnosticsResponse.u8data[0] = 3;
		diagnosticsResponse.u8data[1] = NegativeResponseID;
		diagnosticsResponse.u8data[2] = RequestDownloadService;
		diagnosticsResponse.u8data[3] = ServiceNotSupported;
		diagnosticsResponse.bDataRdy = TRUE;
		return;	
	}
	...
	else if(multipleFramesRx.numBytes != (6 +EXTRA_WRITE_BYTES) )
	{
		diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
		receivedMessageError = TRUE;
	}
	...
	if(receivedMessageError == FALSE)	
	{
		...
		if(multipleFramesRx.mulByteDiagRespData[3] == DATA_FORMATE_IDENTIFIER &&  multipleFramesRx.mulByteDiagRespData[4] == ADD_LENGHT_FORMATE_IDENTIFIER)
		{

		 	 MemroyStartAdd = 	multipleFramesRx.mulByteDiagRespData[5];
			 MemroyStartAdd =  ( (MemroyStartAdd<<8) | multipleFramesRx.mulByteDiagRespData[6] );
			 MemroyStartAdd =   ( (MemroyStartAdd<<8) | multipleFramesRx.mulByteDiagRespData[7] );

			MemroySize = 	multipleFramesRx.mulByteDiagRespData[8];
			MemroySize  =  ( (MemroySize <<8) | multipleFramesRx.mulByteDiagRespData[9] );
			MemroySize  =   ( (MemroySize <<8) | multipleFramesRx.mulByteDiagRespData[10] );	

			if(MemroyStartAdd != APPLICATION_START_ADD || MemroySize != NO_OF_BYTE_OF_APPLICATION)
			{
				receivedMessageError = TRUE;
				diagnosticsResponse.u8data[3] = RequestOutOfRange;			
			}
	...
	{
		diagnosticsResponse.u8data[0] = 4;
		diagnosticsResponse.u8data[1] = 0x74;
		diagnosticsResponse.u8data[2] = 0x20;	// 20 means 2 byte block length 00 FF
		diagnosticsResponse.u8data[3] = (MAX_NUM_BLOCK_LENGHT & 0xff00) >> 8;
		diagnosticsResponse.u8data[4] = (MAX_NUM_BLOCK_LENGHT & 0x00FF);   // ff means MCU can receive upto 256 byte 
		bRequestDownloadServiceActive = TRUE;
	}
...
void TransferData()
{
	...
	if(temp == eApplicationMode)
	{
		diagnosticsResponse.u8data[0] = 3;
		diagnosticsResponse.u8data[1] = NegativeResponseID;
		diagnosticsResponse.u8data[2] = TransferDataService;
		diagnosticsResponse.u8data[3] = ServiceNotSupported;
		diagnosticsResponse.bDataRdy = TRUE;
		return;	
	}

	if(bTranferDataStatusPending == TRUE)
	{
		
		temp = bFlashWriteStatus();
			
			if(temp == eSuccess )
		 	{
			 	diagnosticsResponse.u8data[0] = 2;
				diagnosticsResponse.u8data[1] = 0x76;
				diagnosticsResponse.u8data[2] = u8BlockNumber;				
				diagnosticsResponse.bDataRdy = TRUE;
				bTranferDataStatusPending = 0;
				vClearFlashWriteStatus();
		 	}
			else if(temp == eFail ) // Fail in case of check sum not okay
			{

				diagnosticsResponse.u8data[0] = 3;
				diagnosticsResponse.u8data[1] = 0x7f;
				diagnosticsResponse.u8data[2] = TransferDataService;
				diagnosticsResponse.u8data[3] = GeneralProgrammingFailure;
				diagnosticsResponse.bDataRdy = TRUE;
				bTranferDataStatusPending = 0;
				vClearFlashWriteStatus();
			}
		      ...
		      if( u32GetClockTicks()- ClkTimer > 4000)
		      	{
					...
					if(++PendingResponseCount > 2) 
					{
						...
						diagnosticsResponse.u8data[3] = GeneralProgrammingFailure;
						diagnosticsResponse.bDataRdy = TRUE;
						//tbd which variable and setting reset while programming fail
						bTranferDataStatusPending =0;	
						u8BlockSequenceCounter = 0;
					}
		      	}
			}
	...
	// check is frame sequence error 
   temp =  bGetFrameSequenseErrorStatus();
		
	if(u8BlockSequenceCounter != multipleFramesRx.mulByteDiagRespData[3]) 
	{
		 diagnosticsResponse.u8data[3] = WrongBlockSequenceCounter;
		 receivedMessageError = TRUE;
	}	
	else if(multipleFramesRx.numBytes > (MAX_NUM_BLOCK_LENGHT + EXTRA_WRITE_BYTES) ) //tbd after pratyush code finalize
	{
		diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
		receivedMessageError = TRUE;
	}
	else if(bRequestDownloadServiceActive == FALSE || temp == TRUE)
	{
		diagnosticsResponse.u8data[3] = RequestSequenceError;
		receivedMessageError = TRUE;
		vClearFrameSequenseErrorStatus();
	}
	

	if(receivedMessageError == FALSE)	
	{
		//file data start from location 4 i.e. Frame , No of byte in block , service identifier , block no
		u8BlockNumber = multipleFramesRx.mulByteDiagRespData[3];
		for(i = 0; i < (multipleFramesRx.numBytes - 2); i++)
		{
			MY_RxByteBuffer[i]	= multipleFramesRx.mulByteDiagRespData[i+4];
		}
		
	 	bTranferDataStatusPending = TRUE;

		if(u8BlockSequenceCounter == 0xFF)
		{
			u8BlockSequenceCounter = 0;
		}
		else
		{
		  u8BlockSequenceCounter++;
		}
	...
	else if(bTranferDataStatusPending == TRUE)
	{	
		diagnosticsResponse.u8data[0] = 3;
		diagnosticsResponse.u8data[1] = 0x7f;
		diagnosticsResponse.u8data[2] = TransferDataService;
		diagnosticsResponse.u8data[3] = RequestCorrectlyReceivedResponsePending;
		ClkTimer = u32GetClockTicks(); //reset pending response timer 	
	}
...
void TransferExit()
{
	...
	if(temp == eApplicationMode)
	{
		diagnosticsResponse.u8data[0] = 3;
		diagnosticsResponse.u8data[1] = NegativeResponseID;
		diagnosticsResponse.u8data[2] = TransferExitService;
		diagnosticsResponse.u8data[3] = ServiceNotSupported;
		diagnosticsResponse.bDataRdy = TRUE;
		return;	
	}
		...
		else if(singleFrameRx.numBytes != 1 )
		{
			diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
			receivedMessageError = TRUE;
		}
		else if(receivedMessageError == FALSE)	
		{
			//if need to add sequence error
		}
		...
		{			
			diagnosticsResponse.u8data[0] = 1;
			diagnosticsResponse.u8data[1] = TransferExitResponceService;						
		}
				...
				case LID_TELL_TALE:
					if(multipleFramesRx.mulByteDiagRespData[5] == CONTROL_WITH_TE)
					{
						...
						else
						{
							...
							InputOutputLampControl.IoControlByte[6] = multipleFramesRx.mulByteDiagRespData[12];
		...
		switch(singleFrameRx.dataBytes[3] ) //second byte of the LID
		{
			...
			case LID_DTE_CALC_UPDATE:
				if( receivedMessageError == FALSE )
				{
					...
					diagnosticsResponse.u8data[0] = (DTE_MUL_FACTOR_BYTE-1) + EXTRA_READ_BYTES;
			...
			case LID_TELL_TALE:
				if( receivedMessageError == FALSE )
				{
					...
					multipleFramesTx.numBytes = TELLTALES_OPTION_BYTES + EXTRA_READ_BYTES;
					for(i=0;i<TELLTALES_OPTION_BYTES;i++)
						dataTxBuffer[3+i] = u8TelltalesOption[i];
					mulByteResponse = TRUE;
			...
			case LID_ODOMETER:
				...
				*/
				...
				case LID_FUEL_TANK_CAP:
				if( receivedMessageError == FALSE )
				{
					vGetEolData(eFuelTankCapacity);
					diagnosticsResponse.u8data[0] = FUEL_TANK_CAP_LEN + EXTRA_READ_BYTES;
					
					diagnosticsResponse.u8data[4] = u8FuelTankCapacity[0];
					diagnosticsResponse.u8data[5] = u8FuelTankCapacity[1];
					
					
				}
				break;

			case LID_CAN_SPEEDO_FACTOR:
				if( receivedMessageError == FALSE )
				{					
					vGetEolData(eCanSpeedoFactor);
					multipleFramesTx.numBytes = CAN_SPEEDO_FACTOR_LEN + EXTRA_READ_BYTES;
					
					for(i=0;i<CAN_SPEEDO_FACTOR_LEN;i++)
						dataTxBuffer[3+i] = u8CanSpeedoFactor[i];
					mulByteResponse = TRUE;				
				}
				break;
			/*
		...
		if((multipleFramesRx.mulByteDiagRespData[2*i+3])== LID_GEN_BYTE1)
		{
			switch(multipleFramesRx.mulByteDiagRespData[2*i+4]) //first 4 bytes have: 2 numbytes + read identifier+ first byte of identifier
			{
				...
				case LID_DTE_CALC_UPDATE:
					...
					{
						vGetEolData(eDteMulFactorBytes);
						multipleFramesTx.numBytes += (DTE_MUL_FACTOR_BYTE-1) + EXTRA_READ_MUL_BYTES;
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+3];
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+4];						
				...
				case LID_FUEL_TANK_CAP:
					...
					{
						vGetEolData(eFuelTankCapacity);
						multipleFramesTx.numBytes += FUEL_TANK_CAP_LEN + EXTRA_READ_MUL_BYTES;
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+3];
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+4];
						dataTxBuffer[multiByteIndex++] = u8FuelTankCapacity[0];
						dataTxBuffer[multiByteIndex++] = u8FuelTankCapacity[1];
					}
					break;
				...
				case LID_CAN_SPEEDO_FACTOR:
					...
					{
						vGetEolData(eCanSpeedoFactor);
						multipleFramesTx.numBytes += CAN_SPEEDO_FACTOR_LEN + EXTRA_READ_MUL_BYTES;
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+3];
						dataTxBuffer[multiByteIndex++] = multipleFramesRx.mulByteDiagRespData[2*i+4];
						for(j=0;j<TELLTALES_OPTION_BYTES;j++)
							dataTxBuffer[multiByteIndex++] = u8CanSpeedoFactor[j];
	...
	if(multipleFramesRx.dataReady == TRUE)
	{
			...
			multipleFramesRx.mulByteDiagRespData[4] == LID_VEH_OPT)
		{
			   ...
			   
		   {
			...
			multipleFramesRx.mulByteDiagRespData[4] == LID_CAN_SPEEDO_FACTOR)
			{
				if( receivedMessageError == FALSE )
				{						
					if(multipleFramesRx.numBytes != LID_CAN_SPEEDO_FACTOR + EXTRA_WRITE_BYTES)
					{
						diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
						receivedMessageError = TRUE;
					}
					...
					{
							for(i=0;i<LID_CAN_SPEEDO_FACTOR;i++)
								u8CanSpeedoFactor[i] = multipleFramesRx.mulByteDiagRespData[5+i];
							vSaveEolData(eCanSpeedoFactor);				
					}
				}
			} 
		...
		switch(singleFrameRx.dataBytes[3] ) //second byte of the LID
		{
			...
			case LID_DTE_CALC_UPDATE:
				...
				if( receivedMessageError == FALSE )
				{
					...
					if( singleFrameRx.numBytes != DTE_MUL_FACTOR_BYTE-1 + EXTRA_WRITE_BYTES)
					{
						diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
						receivedMessageError = TRUE;
					}
				...
				if( receivedMessageError == FALSE )
				{
					...
					else
					{
						...
						i = u8GetOdoUpdateByEolCounter();
	 					...
	 					if(tempVar > 9999999 || tempVar < sODO.u32OdoValue || i>=2)
	 					{
							diagnosticsResponse.u8data[3] = RequestOutOfRange;
							receivedMessageError = TRUE;
						}
			...
			case LID_HOURMETER:	
				if( receivedMessageError == FALSE )
				{
					...
					else
					{
						...
						i = u8GetHourUpdateByEolCounter();
	 					...
	 					if(tempVar > 9999999 || tempVar < sHourMeter.u32HourMeter || i>=2)
	 					{
							diagnosticsResponse.u8data[3] = RequestOutOfRange;
							receivedMessageError = TRUE;
						}
				...
				*/
				...
				if( receivedMessageError == FALSE )
				{
					if( singleFrameRx.numBytes != FUEL_TANK_CAP_LEN + EXTRA_WRITE_BYTES)
					{
						diagnosticsResponse.u8data[3] = IncorrectMessageLengthOrInvalidFormat;
						receivedMessageError = TRUE;
					}
					...
					{
						u8FuelTankCapacity[0] = singleFrameRx.dataBytes[4];
						u8FuelTankCapacity[1] = singleFrameRx.dataBytes[5];
						vSaveEolData(eFuelTankCapacity);
...
void sendDiagnosticsResponse (void)
{
	...
	if( diagnosticsResponse.bDataRdy == TRUE )
	{
		...
		vTxMsgBufProcessing(diagnosticsResponse.sMsgID.Identifier,8,diagnosticsResponse.u8data);
	...
	if (multipleFramesTx.dataReady == TRUE )
	{
		...
		{
			dataByteIndex = 0;
			vTxMsgBufProcessing(multipleFramesTx.ID,8,&multipleFramesTx.mulByteDiagRespData[dataByteIndex]);
			diagnosticsTimer = u32GetClockTicks();
	...
	if(multipleFramesRx.frameInProgress == eFirst && multipleFramesRx.status == eWait)
	{
		...
		vTxMsgBufProcessing(ID,8,&frameConrolDataToSend);
	...
	else if(multipleFramesRx.frameInProgress == eTerminate && multipleFramesRx.status == eOverflowAndTerminate)
	{
		...
		vTxMsgBufProcessing(diagnosticsResponse.sMsgID.Identifier,8,&frameConrolDataToSend);
		multipleFramesRx.status = eClearToSend;
		diagnosticsTimer = u32GetClockTicks();
	}
...
void securitySessionTimeOutInit(void)
{
	...
	bRequestDownloadServiceActive = FALSE; // TBD after time out which service and 
	//Must keep u8BlockSequenceCounter = 1 becouse ,trasfer data service start block from 1
	u8BlockSequenceCounter = 1;
	vClearFrameSequenseErrorStatus();
...
u16 u16GetRoutineControlServiceStatus()
{
...
bool bTransferDataServiceStatus()
{
	return bTranferDataStatusPending;
}

bool bIsECUResetByService()
{
	return (bECUResetServiceActive);
}

u8 u8GetIILSecurityAccessStatus()
{
 return securityAccessInfo.IilserverLockStatus;
}

bool bIsCheckSumVarificationDone()
{
	return bCheckSumVarificationDone;
}

void ClearCheckSumVarificationDoneFlag()
{
	bCheckSumVarificationDone = FALSE;
}
