...
u8  u8VehicleOptionContent[VEHICLE_TYPE_BYTES] = {0x4F, 0x7A, 0xC1, 0xE6, 0x00}; //0102
u8	u8VsPulsPerRev[SPEED_SENSOR_BYTES] = {0x1D, 0x28, 0x78, 0x80,0x02};	//012C
u8	u8EngineOverSpeedWarning[ENGINE_OVER_SPEED_BYTES] = {116 , 0x00};	//012D
...
u8	u8DisplayScreenConfig[DISPLAY_SCREEN_CONFIG_BYTES] = {0xfA, 0xff}; //0106
...
u8	u8OdoWriteData[ODO_WRITE_BYTES] = {0x00 , 0x00, 0x00};//0140 //to be removed after TBD with pratyush
...
u8	u8FuelTankCapacity[FUEL_TANK_CAP_LEN] = {0,0}; //first byte Tank capcity and second Dead Valume
u8	u8CanSpeedoFactor[CAN_SPEEDO_FACTOR_LEN] = {0,0,0x0A,0,0x03,0xE8};
...
void vSetEOLSpeedSensorData()
{
	...
	//Read k1 and k2 factor for CAN speedo
	vGetEolData(eCanSpeedoFactor);
	
	temp = (u16)u8CanSpeedoFactor[0]<<8;
	temp |= (u16)u8CanSpeedoFactor[1];
	temp = temp<<8;
	temp |= u8CanSpeedoFactor[2];
	//vSetCanSpeedoK1Factor(temp);	

	temp = (u16)u8CanSpeedoFactor[3]<<8;
	temp |= (u16)u8CanSpeedoFactor[4];
	temp = temp<<8;
	temp |= u8CanSpeedoFactor[5];
	//vSetCanSpeedoK2Factor(temp);
