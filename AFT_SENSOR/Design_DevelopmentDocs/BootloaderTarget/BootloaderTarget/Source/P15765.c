...
static bool bFrameSequenceError = 0;
 ...
 for (RxQuePop = 0; RxQuePop < RX_QUE_SIZE; ++RxQuePop)
 {
	if (diagnosticsBuf[RxQuePop].bDataRdy)
	{
		...
		switch (UDSData.msg15765.NPCI.frameType)
		{
			...
			case eFirst:
				Length =    (u16)UDSData.msg15765.NPCI.numBytes_MSB_SerialNum_FlowStatus << 8;
				  Length =  Length | UDSData.msg15765.NPCI.MulPCI.numBytesLSB;
		        if( multipleFramesTx.status >= eSegmentedTxNotStarted //its an unexpected PDU if segemented transmission is in progress
					&& Length > 7  
					&& Length  < MAX_DATA_LENGTH // number of bytes should be less than receiver buffer size
					)
	        	{
					multipleFramesRx.ID = UDSData.ID; //only the first 2 bytes of ID
					multipleFramesRx.numBytes = Length;
			...
			case eConsecutive:
				if( multipleFramesTx.status != eWait ) //its an unexpected PDU if segemented transmission is in progress
				{
					...
					if((multipleFramesRx.serialNum & 0x0f) != (UDSData.msg15765.dataBytes[0] & 0x0f ))
					{
						/*multipleFramesRx.frameInProgress = eTerminate;
						multipleFramesRx.status = eOverflowAndTerminate;
						numBytesRx = 0;
...
bool bGetFrameSequenseErrorStatus() 
{
	return(bFrameSequenceError);
}

void vClearFrameSequenseErrorStatus() 
{
	bFrameSequenceError = FALSE;
}
