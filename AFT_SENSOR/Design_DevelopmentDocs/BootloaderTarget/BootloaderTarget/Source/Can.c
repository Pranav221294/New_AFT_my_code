...
void vCanInit(void)
{
	...
	_vConfigureCansetting();
	_vCANMsgBufInit();

	_vRxMsgBufInit(BUFFER_NUMBER_0,FILTER_ID_1);
	
	_vRxMsgBufInit(BUFFER_NUMBER_1,FILTER_ID_2);
	_vRxMsgBufInit(BUFFER_NUMBER_2,FILTER_ID_3);
	_vRxMsgBufInit(BUFFER_NUMBER_3,FILTER_ID_4);
	
	_vRxMsgBufInit(BUFFER_NUMBER_4,FILTER_ID_5);
	_vRxMsgBufInit(BUFFER_NUMBER_5,FILTER_ID_6);	
	_vRxMsgBufInit(BUFFER_NUMBER_6,FILTER_ID_7);
	_vRxMsgBufInit(BUFFER_NUMBER_7,FILTER_ID_8);
	
	_vRxMsgBufInit(BUFFER_NUMBER_8,FILTER_ID_9);	 
	_vRxMsgBufInit(BUFFER_NUMBER_9,FILTER_ID_10);	
	_vRxMsgBufInit(BUFFER_NUMBER_10,FILTER_ID_11);
	
	_vRxMsgBufInit(BUFFER_NUMBER_11,FILTER_ID_12);
	_vRxMsgBufInit(BUFFER_NUMBER_12,FILTER_ID_13);
	_vRxMsgBufInit(BUFFER_NUMBER_13,FILTER_ID_14);
...
//In bootloader code Msk are not unsig 
/*
		C0MASK1L = (u16)MASK_1;
...
/*********************************************************************//**
 *
 * Re-initialization CAN module  , when user want to change baud rate in running time 
 *
 * @param      None    
 *
 * @return     None
...
//No need to correlate filter with mask because ID's very less

*(( u8 *)(MsgBuf_address + 0x09)) = 0x09;   // Receive message buffer(no mask), MA0=1,msg buffer used;C0MCONFm

/*
	if(buffer_number == 10)
	{
		*(( u8 *)(MsgBuf_address + 0x09)) = 0x11;    	// Receive message buf linked with mask 1,, MA0=1,msg buffer used
	}
	else if (buffer_number == 11)
	{
		*(( u8 *)(MsgBuf_address + 0x09)) = 0x19;   // Receive message buffer(mask 2), MA0=1,msg buffer used
	}
	else if ( buffer_number==12 )
	{
		*(( u8 *)(MsgBuf_address + 0x09)) = 0x21;   // Receive message buffer(mask 3), MA0=1,msg buffer used
	}
	else if (buffer_number==13)
	{
		*(( u8 *)(MsgBuf_address + 0x09)) = 0x29;   // Receive message buffer(mask 3), MA0=1,msg buffer used
	}
	else
	{
		*(( u8 *)(MsgBuf_address + 0x09)) = 0x09;   // Receive message buffer(no mask), MA0=1,msg buffer used;C0MCONFm
...
{
	C0CTRL = 0x0810U;   					// set PSMODE0=1,PSMODE1=0, setting CAN sleep mode
...
void MY_CAN_StopMode_Setting(void)
{
	...
	C0CTRL = 0x1800U;   			// set PSMODE0=1,PSMODE1=1, setting CAN stop mode
	while((C0CTRL&0x0010) == 0U); 		// check PSMODE1=1
}
/*********************************************************************//**
*
...
{
	C0CTRL = 0x0810U;   			// clear PSMODE1=0, release stop mode to sleep mode
}
...
{
	C0CTRL = 0x0008U;    			// clear PSMODE0=0,release sleep mode by software
